...
void gfx::TerrainPipeline::cleanupPipelineLayout() {
    VkDevice device = m_renderer->system()->device();
...
void gfx::TerrainPipeline::cleanupPipeline() {
    VkDevice device = m_renderer->system()->device();
...
template<typename T>
void gfx::TerrainPipeline::initGeometryBuffer(VkBuffer &dst_buffer, VkDeviceMemory &dst_memory, const std::vector<T> &data, VkBufferUsageFlags usage) {
    System *system = m_renderer->system();
    VkDevice device = system->device();
    ...
    system->copyBuffer(dst_buffer, staging_buffer, buffer_size);
...
VkShaderModule gfx::TerrainPipeline::createShaderModule(const Resource &rsrc) {
    VkDevice device = m_renderer->system()->device();
